...
# F1 Gear Mobile Optimizations Script
# This script updates HTML files to include mobile optimization features
...
echo -e "${YELLOW}F1 Gear Mobile Optimization Script${NC}"
echo "This script will update your HTML files with mobile optimization features."
echo

# Create mobile-optimizations.js file
echo -e "${GREEN}Creating mobile-optimizations.js file...${NC}"

cat > mobile-optimizations.js << 'EOF'
...
echo -e "${GREEN}Successfully created mobile-optimizations.js${NC}"

# Mobile filter HTML code
FILTER_HTML='<!-- Mobile Filter Button -->
<button id="mobile-filter-button" aria-label="Filter products">
  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<!-- Script is added at the end of this function -->'

# Script tag to add
SCRIPT_TAG='<script src="mobile-optimizations.js"></script>'

# Function to update HTML files
update_html_file() {
    local file=$1
    echo -e "Updating ${YELLOW}$file${NC}..."
    ...
    # Check if mobile filter elements already exist
    if grep -q "id=\"mobile-filter-button\"" "$file"; then
        echo -e "${YELLOW}Mobile filter button already exists in $file. Skipping filter button insertion.${NC}"
    else
        # Insert mobile filter HTML before the closing body tag
        sed -i '' -e "/<\/body>/i\\
$FILTER_HTML
" "$file"
        echo -e "${GREEN}Added mobile filter components to $file${NC}"
    fi

    # Check if the script tag already exists
    if grep -q "mobile-optimizations.js" "$file"; then
        echo -e "${YELLOW}Script tag already exists in $file. Skipping script tag insertion.${NC}"
    else
        # Insert script tag before the closing body tag
        sed -i '' -e "/<\/body>/i\\
$SCRIPT_TAG
" "$file"
        echo -e "${GREEN}Added script tag to $file${NC}"
    fi
}

# Update all HTML files
echo -e "\n${GREEN}Updating HTML files...${NC}"
...
echo -e "\n${GREEN}Mobile optimization complete!${NC}"
echo "Original files have been backed up with .bak extension."
echo -e "You can now open your HTML files to see the improved mobile experience."